...
public class Turret implements Subsystem {
    ...
    private static final double H_TICKS_PER_DEGREES = 483000 / 90;
    ...
    // Elevation servo constants
    public static final double TURRET_SIDE_A = 244.475;  // from CAD team for trig, millimeters
    public static final double TURRET_SIDE_B = 369.4176;
    private final double V_MIN_ANGLE = 20.0; // Needed if we set elevation from the vision system
    ...
    public void periodic() { 
        SmartDashboard.putNumber("Turret Elevation", getCurrentVerticalAngle());
    ...
    // Getters and setters for the turret status variable that stores the vision status
    public void setTurretStatus(String input) {
        visionStatus = input;
    }
    ...
    // Turns on and off the vision lights
    ...
    // Will be used if we add elevation changing
    ...
    // Used to reset encoder ticks if it's at a limit switch
    ...
    // Given a setpoint in degrees, return true if the angle is reached
    public boolean gotoHorizontalAngle(double setpoint) {
        SmartDashboard.putNumber("Turret Angle To Setpoint", setpoint);
        SmartDashboard.putBoolean("Turret Valid Angle Setpoint",
            getCurrentHorizontalAngle() + setpoint < H_MIN_DEGREES
            || getCurrentHorizontalAngle() + setpoint > H_MAX_DEGREES
        );

        // If it's within the deadband...
        if (Math.abs(setpoint) > H_TOLERANCE) {
            // Scale the speed using a bang-bang controller
    ...
    // Accept input from -1 to 1, where -1 in -90 degrees and 1 is 90 degrees
    public boolean runHorizontalManual(double target) {
        // Normalize ticks to a number between 0 and 1
    ...
    public double getCurrentVerticalAngle() {
        ...
        double angle = Math.toDegrees(Math.acos((Math.pow(d, 2) - Math.pow(TURRET_SIDE_A, 2) - Math.pow(TURRET_SIDE_B, 2))/(-2*TURRET_SIDE_A*TURRET_SIDE_B)));
        return angle;
    }
